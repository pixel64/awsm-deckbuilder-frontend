...
let CardSearchModuleComponent = class CardSearchModuleComponent {
    constructor(cardSearchService) {
...
let DecklistComponent = class DecklistComponent {
    constructor(storageService) {
        this.storageService = storageService;
...
let CardSearchService = class CardSearchService {
    constructor(http) {
        this.http = http;
    }
    getCard(cardname) {
...
let ListStorageService = class ListStorageService {
    constructor() {
    ...
    addCardToHashmap(card) {
        const cardPileCount = this.realCards.get(card.id).count + 1;
    ...
    saveList() {
        this.db.update('decklist', { id: 1, key: 'decklist', list: this.mapToJson(this.realCards) }).then(() => {
        }, error => {
            console.error(error);
        });
    }
    ...
    initList() {
        this.db.getByKey('decklist', 1).then(list => {
            const cardMap = this.jsonToMap(list.list);
            this.realCards = cardMap;
